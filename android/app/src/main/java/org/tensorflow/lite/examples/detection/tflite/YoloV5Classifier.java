...
//                    options.setAllowBufferHandleOutput(true);
            ...
            if (isGPU) {
                // 수정된 코드 (GpuDelegate 사용)
                GpuDelegate.Options gpuOptions = new GpuDelegate.Options();
                // 주석 처리 gpuOptions.setPrecisionLossAllowed(true); // It seems that the default is true
                // 주석 처리 gpuOptions.setInferencePreference(GpuDelegate.Options.INFERENCE_PREFERENCE_SUSTAINED_SPEED);
...
//        int[] intValues = new int[INPUT_SIZE * INPUT_SIZE];
    ...
    public ArrayList<Recognition> recognizeImage(Bitmap bitmap) {
        ...
        HashMap<String,String> nameMap = new HashMap<String,String>(){{
            put("aloe", "자연은  내사랑  알로에");
            put("bacchus", "박카스");
            put("cider", "나랑드  사이다");
            put("coca", "코카콜라");
            put("coco1", "코코팜  피치핑크  복숭아");
            put("coco2", "코코팜  화이트  요구르트");
            put("coffee", "디앤카페  오리지널");
            put("confid", "컨피던스");
            put("demi", "데미소다  사과");
            put("fanta", "환타  파인애플 향");
            put("maesil", "초록  매실");
            put("mango", "프레주  망고  스위트");
            put("olatte", "오라떼  딸기");
            put("orange", "자연은  고칼슘  오렌지");
            put("peach", "프레주  복숭아 티");
            put("pocari", "포카리스웨트");
            put("sikhye1", "동원   정성껏  빚은  식혜");
            put("sikhye2", "비락  식혜");
        }};
